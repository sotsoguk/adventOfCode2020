...
def identify_fields(rules, valid_tickets):
    ...
    for i in range(num_fields):
        pos.append(set([valid_tickets[n][i]
                        for n in range(len(valid_tickets))]))
    ...
    for i, p in enumerate(pos):
        for rule_name, rule_range in rules.items():
            ...
            # <= works for sets as issubset (< is a strict subset)
            if p <= rule_range:
...
def parse_input_16(lines):
    ...
    for i, l in enumerate(lines):
        if l == "":
            ...
            range_set = set(range(nums[0], nums[1]+1)
                            ).union(set(range(nums[2], nums[3]+1)))
...
def main():
    ...
    # parse input
    rules, ticket, nearby = parse_input_16(lines)
    ...
    part2 = reduce(mul, [ticket[v]
                         for k, v in rcd.items() if k.startswith('departure')], 1)
